...
/**
 * Auto-generate articles script for bermo.ro
...
// Config - 1 article per run for bermo.ro
...
async function main() {
  await log('='.repeat(60));
  await log('AUTO-GENERATE STARTED - bermo.ro');
  await log('='.repeat(60));
  ...
  try {
    ...
    // Try reading generation-results.json as fallback
    try {
      const resultsPath = path.join(projectDir, 'generation-results.json');
      const results = JSON.parse(await fs.readFile(resultsPath, 'utf-8'));
      successfulKeywords = results.filter(r => r.success).map(r => r.keyword);
    } catch (e2) {
  ...
  const projectName = process.env.CLOUDFLARE_PROJECT_NAME || 'bermo-ro';
